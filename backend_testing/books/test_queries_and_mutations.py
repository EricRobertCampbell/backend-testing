...
class TestCreatBookWithExistingAuthor(GraphQLTestCase):
    ...
    def test_fail_with_missing_params(self) -> None:
        ...
        self.assertEqual(
            response.status_code,
            400,
            f"Unexpected status code for syntactically bad query: {decoded}",
        )
        self.assertIn(
            "errors",
            decoded,
            f"Unexpected lack of 'errors' key in response to bad query: {decoded}",
        )

    def test_fail_with_missing_author(self) -> None:
        """Author doesn't exist -> fail"""
        query_variables = {
            "firstName": "Some",
            "lastName": "Guy",
            "title": "Clean Code",
    ...
    def test_create_book_with_existing_author_should_work(self) -> None:
        ...
        self.assertEqual(
            author,
            {
                "firstName": query_variables["firstName"],
                "lastName": query_variables["lastName"],
            },
